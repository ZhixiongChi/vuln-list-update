...
var (
	...
	esaHeader = regexp.MustCompile(`^\[(\d\d) ([A-Z][a-z][a-z]) (\d{4})\] ` +
		`(?P<id>ESA-\d+(?:-\d+)?)\s+` +
		`(?P<description>.*?)\s*$`,
	)
...
func (l esaList) ParseHeader(line string) *Header {
	results := esaHeader.FindStringSubmatch(line)
	if len(results) == 0 {
		return nil
	}

	id := results[esaHeader.SubexpIndex("id")]
	description := results[esaHeader.SubexpIndex("description")]
